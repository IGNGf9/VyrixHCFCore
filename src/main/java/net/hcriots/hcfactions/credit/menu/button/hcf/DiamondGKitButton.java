...
@AllArgsConstructor
public class DiamondGKitButton extends Button {

    private static final int PRICE = 50;

    @Override
    public String getName(Player player) {
        return ChatColor.DARK_RED.toString() + "One Time Use Diamond Gkit";
    }
    ...
    public void clicked(Player player, int i, ClickType clickType) {
        if (clickType.isRightClick() || clickType.isLeftClick()) {
            ...
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "gkit Diamond " + player.getName());
