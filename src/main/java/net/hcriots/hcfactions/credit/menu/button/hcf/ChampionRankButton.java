...
@AllArgsConstructor
public class ChampionRankButton extends Button {

    private static final int PRICE = 120;

    @Override
    public String getName(Player player) {
        return ChatColor.GOLD.toString() + ChatColor.BOLD + "30 Day Champion Rank";
    }
    ...
    public void clicked(Player player, int i, ClickType clickType) {
        if (clickType.isRightClick() || clickType.isLeftClick()) {
            ...
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "ogrant " + player.getName() + " Champion global 30d Charms Buyer");
