...
@AllArgsConstructor
public class CheckmarkPrefixButton extends Button {

    private static final int PRICE = 100;

    @Override
    public String getName(Player player) {
        return ChatColor.GREEN.toString() + "Permanent Checkmark Prefix";
    }
    ...
    public void clicked(Player player, int i, ClickType clickType) {
        if (clickType.isRightClick() || clickType.isLeftClick()) {
            ...
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "player addperm " + player.getName() + " tag.checkmark");
