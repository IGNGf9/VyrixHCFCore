...
public class Claim implements Iterable<Coordinate> {
    ...
    public BasicDBObject json() {
        ...
        World world = Hulu.getInstance().getServer().getWorld(getWorld());
    ...
    public Location getMinimumPoint() {
        return (new Location(Hulu.getInstance().getServer().getWorld(world), Math.min(x1, x2), Math.min(y1, y2), Math.min(z1, z2)));
    }

    public Location getMaximumPoint() {
        return (new Location(Hulu.getInstance().getServer().getWorld(world), Math.max(x1, x2), Math.max(y1, y2), Math.max(z1, z2)));
    }
    ...
    public Location[] getCornerLocations() {
        World world = Hulu.getInstance().getServer().getWorld(this.world);
