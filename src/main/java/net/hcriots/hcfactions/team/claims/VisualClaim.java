...
import net.hcriots.hcfactions.team.claims.Claim.CuboidDirection;
import net.hcriots.hcfactions.team.commands.team.TeamClaimCommand;
import net.hcriots.hcfactions.team.commands.team.TeamResizeCommand;
...
public class VisualClaim implements Listener {
    ...
    @Getter
    private static final Map<String, VisualClaim> currentSubclaimMaps = new HashMap<>();
    @Getter
    private static final Map<String, VisualClaim> visualClaims = new HashMap<>();
    private static final Map<UUID, Queue<QueuedBlockChange>> queuedBlockChanges = Maps.newHashMap();
    ...
    @Getter
    @NonNull
    private final VisualClaimType type;
    @Getter
    @NonNull
    ...
    private static void checkTaskSetup() {
        ...
        taskSetup = true;
        Bukkit.getScheduler().runTaskTimer(Hulu.getInstance(), () -> {
    ...
    public void draw(boolean silent) {
        ...
        switch (type) {
            case CREATE:
                ...
                if (!bypass && Hulu.getInstance().getMapHandler().isKitMap()) {
                    if (!silent) {
            ...
            case MAP:
                ...
                Map<Entry<Claim, Team>, Material> sendMaps = new HashMap<>();

                for (Entry<Claim, Team> regionData : LandBoard.getInstance().getRegionData(player.getLocation(), MAP_RADIUS, 256, MAP_RADIUS)) {
                ...
                if (!silent) {
                    for (Entry<Entry<Claim, Team>, Material> mapEntry : sendMaps.entrySet()) {
                        Team team = mapEntry.getKey().getValue();
                        Claim claim = mapEntry.getKey().getKey();

                        if (team.getOwner() == null) {
                            player.sendMessage(ChatColor.YELLOW + "Pillars " + ChatColor.GREEN + "(" + ChatColor.AQUA + ItemUtils.getName(new ItemStack(mapEntry.getValue())) + ChatColor.GREEN + ") " +
                                    ChatColor.YELLOW + "is claimed by " + ChatColor.BLUE + team.getName(player));
                        } else {
                            player.sendMessage(
                                    ChatColor.YELLOW + "Pillars " + ChatColor.GREEN + "(" + ChatColor.AQUA + ItemUtils.getName(new ItemStack(mapEntry.getValue())) + ChatColor.GREEN + ") " + ChatColor.YELLOW +
                                            "is claimed by " + ChatColor.BLUE + team.getName());
                        }
                    }
                }
            ...
            case SURFACE_MAP:
                for (Entry<Claim, Team> regionData : LandBoard.getInstance().getRegionData(player.getLocation(), MAP_RADIUS, 256, MAP_RADIUS)) {
                    ...
                    World claimWorld = Hulu.getInstance().getServer().getWorld(claim.getWorld());
                ...
                break;
            case SUBCLAIM_MAP:
                Team senderTeam = Hulu.getInstance().getTeamHandler().getTeam(player);
                ...
                if (!silent) {
                    for (Entry<Subclaim, Material> entry : subclaimMaterialMap.entrySet()) {
                        player.sendMessage(ChatColor.YELLOW + "Subclaim " + ChatColor.BLUE + entry.getKey().getName() + ChatColor.GREEN + "(" + ChatColor.AQUA + ItemUtils.getName(new ItemStack(entry.getValue())) + ChatColor.GREEN + ") " +
                                ChatColor.YELLOW + "is claimed by " + ChatColor.BLUE + senderTeam.getName());
                    }
                }
    ...
    public boolean containsOtherClaim(Claim claim) {
        ...
        for (int x = minPoint.getBlockX(); x <= maxPoint.getBlockX(); x++) {
            for (int z = minPoint.getBlockZ(); z <= maxPoint.getBlockZ(); z++) {
                Location at = new Location(Hulu.getInstance().getServer().getWorld(claim.getWorld()), x, 80, z);
    ...
    public Set<Claim> getTouchingClaims(Claim claim) {
        ...
        for (Coordinate coordinate : claim.outset(CuboidDirection.Horizontal, 1)) {
            Location loc = new Location(Hulu.getInstance().getServer().getWorld(claim.getWorld()), coordinate.getX(), 80, coordinate.getZ());
            Entry<Claim, Team> claimAtLocation = LandBoard.getInstance().getRegionData(loc);
    ...
    public void setLoc(int locationId, final Location clicked) {
        ...
        if (type == VisualClaimType.CREATE) {
            if (!bypass && !Hulu.getInstance().getServerHandler().isUnclaimed(clicked)) {
            ...
            Hulu.getInstance().getServer().getScheduler().runTaskLater(Hulu.getInstance(), new BukkitRunnable() {
            ...
            player.sendMessage(
                    ChatColor.YELLOW + "Set claim's location " + ChatColor.LIGHT_PURPLE + locationId + ChatColor.YELLOW + " to " + ChatColor.GREEN + "(" + ChatColor.WHITE + clicked.getBlockX() + ", " + clicked.getBlockY() + ", " +
            ...
            if (corner1 != null && corner2 != null) {
                ...
                if (price > playerTeam.getBalance() && !bypass) {
                    player.sendMessage(ChatColor.YELLOW + "Claim cost: " + ChatColor.RED + "$" + price + ChatColor.YELLOW + ", Current size: (" + ChatColor.WHITE + x + ", " + z + ChatColor.YELLOW + "), " + ChatColor.WHITE + (x * z) +
                            ChatColor.YELLOW + " blocks");
                } else {
                    player.sendMessage(ChatColor.YELLOW + "Claim cost: " + ChatColor.GREEN + "$" + price + ChatColor.YELLOW + ", Current size: (" + ChatColor.WHITE + x + ", " + z + ChatColor.YELLOW + "), " + ChatColor.WHITE + (x * z) +
                            ChatColor.YELLOW + " blocks");
                }
            }
        } else if (type == VisualClaimType.RESIZE) {
            Entry<Claim, Team> teamAtLocation = LandBoard.getInstance().getRegionData(clicked);
            ...
            if (locationId == 1) {
                ...
                player.sendMessage(
                        ChatColor.YELLOW + "Set resize location to " + ChatColor.GREEN + "(" + ChatColor.WHITE + clicked.getBlockX() + ", " + clicked.getBlockY() + ", " + clicked.getBlockZ() + ChatColor.GREEN + ")" + ChatColor.YELLOW +
                                ".");
            }
    ...
    public void purchaseClaim() {
        ...
        if (corner1 != null && corner2 != null) {
            ...
            claim.setName(playerTeam.getName() + "_" + (100 + ThreadLocalRandom.current().nextInt(800)));
            claim.setY1(0);
            claim.setY2(256);
            ...
            if (!bypass) {
                ...
                Hulu.getInstance().getLogger().info("Economy Logger: Withdrawing " + price + " from " + playerTeam.getBalance() + "'s account: Claimed land");
