...
public class PotionStorageButton extends Button {
    ...
    public List<String> getDescription(@NotNull Player player) {
        return Arrays.asList(
                "&7&m-------------------------------------------",
                "&7Click to view stored items.",
                "&7&m-------------------------------------------");
    }

    @Override
    public ItemStack getButtonItem(@NotNull Player player) {
        return new ItemBuilder(Material.CHEST).name("&6&lStorage").setLore(getDescription(player)).build();
    }

    @Override
    public void clicked(Player player, int i, ClickType clickType) {
        if (clickType.isLeftClick()) {
            Bukkit.dispatchCommand(player, "f brewinv");
        }
    }
