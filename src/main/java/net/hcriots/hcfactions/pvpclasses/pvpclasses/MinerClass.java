...
public class MinerClass extends PvPClass implements Listener {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                for (String key : new HashMap<>(noDamage).keySet()) {
                    ...
                    Player player = Hulu.getInstance().getServer().getPlayerExact(key);
    ...
    public void tick(Player player) {
        ...
        int diamonds = Hulu.getInstance().getDiamondMinedMap().getMined(player.getUniqueId());
        int level = 1;
        if (diamonds > 125) {
            level = 3;
        } else if (diamonds > 50) {
    ...
    public void onPlayerPickupItem(PlayerPickupItemEvent event) {
        ...
        if (!Hulu.getInstance().getCobblePickupMap().isCobblePickup(player.getUniqueId())) {
