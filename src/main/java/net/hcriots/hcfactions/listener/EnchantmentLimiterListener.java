...
public class EnchantmentLimiterListener implements Listener {
    ...
    private final Map<String, Long> lastArmorCheck = new HashMap<>();
    private final Map<String, Long> lastSwordCheck = new HashMap<>();
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        if (event.getEntity() instanceof Player && checkArmor((Player) event.getEntity())) {
            ...
            if (fixed) {
                boolean value = Hulu.getInstance().getLanguageMap().isNewLanguageToggle(event.getEntity().getUniqueId());

                ((Player) event.getEntity()).sendMessage((value ? ChatColor.RED + "We detected that your armor had some illegal enchantments, and have reduced the invalid enchantments." : ChatColor.RED + "Detectamos encantamientos ilegales, y reducimos el encantamiento."));
            }
        }
    }
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        if (event.getDamager() instanceof Player && checkSword((Player) event.getDamager())) {
            ...
            if (InventoryUtils.conformEnchants(hand)) {
                ...
                player.sendMessage((value ? ChatColor.RED + "We detected that your sword had some illegal enchantments, and have reduced the invalid enchantments." : ChatColor.RED + "Detectamos que tu espada tiene algunos encantamientos ilegales."));
            }
        }
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if ((event.getAction() == Action.RIGHT_CLICK_BLOCK || event.getAction() == Action.RIGHT_CLICK_AIR) && event.getItem() != null && event.getItem().getType() == Material.BOW) {
            ...
            if (InventoryUtils.conformEnchants(hand)) {
                ...
                event.getPlayer().sendMessage((value ? ChatColor.RED + "We detected that your bow had some illegal enchantments, and have reduced the invalid enchantments." : ChatColor.RED + "Detectamos que tu arco tiene algunos encantamientos ilegales, y reducimos esos encantamientos."));
            }
        }
    }
        ...
        if (event.getInventory() instanceof MerchantInventory) {
            ...
            if (meta != null && meta.hasDisplayName()) {
                ...
                if (previous != null && previous.hasItemMeta() && previous.getItemMeta().hasDisplayName() && containsColor(previous.getItemMeta().getDisplayName())) {
                    ...
                    }.runTaskLater(Hulu.getInstance(), 6L);
