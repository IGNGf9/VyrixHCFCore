...
public class CombatLoggerListener implements Listener {
    ...
    @Getter
    private final Set<Entity> combatLoggers = new HashSet<>();
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        if (event.getEntity().hasMetadata(COMBAT_LOGGER_METADATA)) {
            ...
            if (!metadata.playerName.equals(event.getEntity().getCustomName().substring(2))) {
                Hulu.getInstance().getLogger().warning("Combat logger name doesn't match metadata for " + metadata.playerName + " (" + event.getEntity().getCustomName().substring(2) + ")");
            }

            Hulu.getInstance().getLogger().info(metadata.playerName + "'s combat logger at (" + event.getEntity().getLocation().getBlockX() + ", " + event.getEntity().getLocation().getBlockY() + ", " + event.getEntity().getLocation().getBlockZ() + ") died.");

            // Deathban the player
            Hulu.getInstance().getDeathbanMap().deathban(metadata.playerUUID, metadata.deathBanTime);

            // Increment players deaths
            int deaths = Hulu.getInstance().getDeathsMap().getDeaths(metadata.playerUUID);
            Hulu.getInstance().getDeathsMap().setDeaths(metadata.playerUUID, deaths + 1);

            Team team = Hulu.getInstance().getTeamHandler().getTeam(metadata.playerUUID);

            // Take away DTR.
            if (team != null) {
                team.playerDeath(metadata.playerName, Hulu.getInstance().getServerHandler().getDTRLoss(event.getEntity().getLocation()));
            }
            ...
            // give them a death
            if (isKitMap) {
                Hulu.getInstance().getMapHandler().getStatsHandler().getStats(metadata.playerUUID).addDeath();
            }


            // store the death amount -- we'll use this later on.
            int victimKills = Hulu.getInstance().getKillsMap().getKills(event.getEntity().getUniqueId());

            if (isKitMap) {
                victimKills = Hulu.getInstance().getMapHandler().getStatsHandler().getStats(event.getEntity().getUniqueId()).getKills();
            }

            if (event.getEntity().getKiller() != null) {
                // give them a kill
                Hulu.getInstance().getKillsMap().setKills(event.getEntity().getKiller().getUniqueId(), Hulu.getInstance().getKillsMap().getKills(event.getEntity().getKiller().getUniqueId()) + 1);

                // store the kill amount -- we'll use this later on.
                int killerKills = Hulu.getInstance().getKillsMap().getKills(event.getEntity().getKiller().getUniqueId());

                if (isKitMap) {
                    Hulu.getInstance().getMapHandler().getStatsHandler().getStats(event.getEntity().getKiller()).addKill();

                    killerKills = Hulu.getInstance().getMapHandler().getStatsHandler().getStats(event.getEntity().getKiller()).getKills();
                }
                ...
                for (Player player : Bukkit.getOnlinePlayers()) {
                    if (Hulu.getInstance().getToggleDeathMessageMap().areDeathMessagesEnabled(player.getUniqueId())) {
                        ...
                        if (Hulu.getInstance().getTeamHandler().getTeam(player.getUniqueId()) == null) {
                            continue;
                        }

                        if (Hulu.getInstance().getTeamHandler().getTeam(metadata.playerUUID) != null
                                && Hulu.getInstance().getTeamHandler().getTeam(metadata.playerUUID).equals(Hulu.getInstance().getTeamHandler().getTeam(player.getUniqueId()))) {
                            player.sendMessage(deathMessage);
                        }

                        if (Hulu.getInstance().getTeamHandler().getTeam(event.getEntity().getKiller().getUniqueId()) != null
                                && Hulu.getInstance().getTeamHandler().getTeam(event.getEntity().getKiller().getUniqueId()).equals(Hulu.getInstance().getTeamHandler().getTeam(player.getUniqueId()))) {
                            player.sendMessage(deathMessage);
                        }
                    }
                }
...
//              }
            ...
            Player target = Hulu.getInstance().getServer().getPlayer(metadata.playerUUID);

            if (target == null) {
                // Create an entity to load the player data
                MinecraftServer server = ((CraftServer) Hulu.getInstance().getServer()).getServer();
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        ...
        if (damager != null) {
            ...
            if (Hulu.getInstance().getServerHandler().isSpawnBufferZone(event.getEntity().getLocation())) {
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        if (event.getPlayer().hasMetadata("loggedout")) {
            event.getPlayer().removeMetadata("loggedout", Hulu.getInstance());
            return;
        }
        ...
        if (event.getPlayer().hasMetadata("frozen") || Stark.instance.getServerHandler().getFrozen()) {
            return;
        }
        ...
        for (Entity entity : event.getPlayer().getNearbyEntities(40, 40, 40)) {
            if (entity instanceof Player) {
                ...
                Team otherTeam = Hulu.getInstance().getTeamHandler().getTeam(other);
        ...
        if (event.getPlayer().getGameMode() != GameMode.CREATIVE && !event.getPlayer().hasMetadata("invisible") && spawnCombatLogger && !event.getPlayer().isDead()) {
            Hulu.getInstance().getLogger().info(event.getPlayer().getName() + " combat logged at (" + event.getPlayer().getLocation().getBlockX() + ", " + event.getPlayer().getLocation().getBlockY() + ", " + event.getPlayer().getLocation().getBlockZ() + ")");
            ...
            metadata.deathBanTime = Hulu.getInstance().getServerHandler().getDeathban(metadata.playerUUID, event.getPlayer().getLocation());
            metadata.contents = inv;
            metadata.armor = armor;

            villager.setMetadata(COMBAT_LOGGER_METADATA, new FixedMetadataValue(Hulu.getInstance(), metadata));
            ...
            }.runTaskLater(Hulu.getInstance(), 30 * 20L);
            ...
            if (villager.getWorld().getEnvironment() == World.Environment.THE_END) {
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        Hulu.getInstance().getLogger().info(metadata.playerName + "'s combat logger at (" + villager.getLocation().getBlockX() + ", " + villager.getLocation().getBlockY() + ", " + villager.getLocation().getBlockZ() + ") died.");
                        ...
                        // Take away DTR.
                        if (team != null) {
                            team.playerDeath(metadata.playerName, Hulu.getInstance().getServerHandler().getDTRLoss(villager.getLocation()));
                        }

                        // store the death amount -- we'll use this later on.
                        int victimKills = Hulu.getInstance().getKillsMap().getKills(metadata.playerUUID);

                        if (Hulu.getInstance().getMapHandler().isKitMap()) {
                            victimKills = Hulu.getInstance().getMapHandler().getStatsHandler().getStats(metadata.playerUUID).getKills();
                        }
                        ...
                        for (Player player : Bukkit.getOnlinePlayers()) {
                            if (Hulu.getInstance().getToggleDeathMessageMap().areDeathMessagesEnabled(player.getUniqueId())) {
                                ...
                                if (team != null && team == Hulu.getInstance().getTeamHandler().getTeam(player.getUniqueId())) {
                                    player.sendMessage(deathMessage);
                                }
                            }
                        }
