...
public class MapListener implements Listener {
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        double multiplier = Hulu.getInstance().getMapHandler().getBaseLootingMultiplier();
        ...
        if (event.getEntity().getKiller() != null) {
            ...
            if (player.getItemInHand() != null && player.getItemInHand().containsEnchantment(Enchantment.LOOT_BONUS_MOBS)) {
                switch (player.getItemInHand().getEnchantmentLevel(Enchantment.LOOT_BONUS_MOBS)) {
                    case 1:
                        multiplier = Hulu.getInstance().getMapHandler().getLevel1LootingMultiplier();
                        break;
                    case 2:
                        multiplier = Hulu.getInstance().getMapHandler().getLevel2LootingMultiplier();
                        break;
                    case 3:
                        multiplier = Hulu.getInstance().getMapHandler().getLevel3LootingMultiplier();
                        break;
                    default:
                        break;
                }
    ...
    // This is actually 'Lowest', but Bukkit calls listeners LOWEST -> HIGHEST, so HIGHEST is what's actually called last. #BukkitBeLike
    public void onBlockBreak(BlockBreakEvent event) {
        if (!Hulu.getInstance().getMapHandler().isFastSmeltEnabled() || event.getPlayer().getItemInHand() == null || !event.getPlayer().getItemInHand().getType().name().contains("PICKAXE") || event.getPlayer().getItemInHand().containsEnchantment(Enchantment.SILK_TOUCH)) {
            return;
        }
    ...
    public void onProjectileLaunch(ProjectileLaunchEvent event) {
        ...
        if (inHand != null && inHand.getType() == Material.EXP_BOTTLE && inHand.hasItemMeta() && inHand.getItemMeta().hasLore() && inHand.getItemMeta().getLore().size() == 1) {
            ...
            thrown.setMetadata("XP", new FixedMetadataValue(Hulu.getInstance(), xp));
        }
    }
    ...
    public void onSplash(ExpBottleEvent event) {
        ...
        if (bottle.hasMetadata("XP")) {
            ...
            bottle.removeMetadata("XP", Hulu.getInstance());
        }
    }
